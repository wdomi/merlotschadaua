/***************************************************************************
 * Merlotschadaua – Full Working app.js (2025 edition)
 * Completely rebuilt for new layout, mobile layout, AND-logic filtering,
 * color-coded UI, CSV parsing fixed for Firefox, PWA safe.
 **************************************************************************/
...
const DEFAULT_CENTER = [46.7000, 10.0833]; // fallback Zernez
const CSV_URL = "/data/view_birdsCSV_apps.csv";

const OFFLINE_QUEUE_KEY = "merlotschadaua_offline_queue";

// Fixed color palette (aligned with your buttons)
const COLOR_PALETTE = {
  alu:    "#808080",
  white:  "#eee",
  red:    "#e22c22",
...
function stripBOM(text) {
  // Firefox sometimes includes BOM
  return text.replace(/^\uFEFF/, "");
}
...
function parseCSV(text) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    matches.forEach(x => cols.push(x.replace(/^"|"$/g, "").trim()));
...
function buildColorButtons() {
  ...
  COLOR_ORDER.forEach(color => {
    ...
    br.style.color = (color === "white" ? "#000" : "#fff");
    ...
    bl.style.color = (color === "white" ? "#000" : "#fff");
...
// ------------------------------------------------------------------------
// FILTERING LOGIC (AND logic per leg)
// ------------------------------------------------------------------------
...
function colorSpan(c) {
  if (!c) return "";
  const hex = COLOR_PALETTE[c] || "#444";
  const col = (c === "white" ? "#000" : "#fff");
  return `<span style="
      background:${hex};
      color:${col};
      padding:2px 4px;
