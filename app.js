/***************************************************************************
 * Merlotschadaua – FINAL OFFLINE-COMPATIBLE APP.JS (2025-12-03)
 * - Auto offline queue
 * - Auto resend on reconnect
 * - Map popup freeze fix
 * - Correct lat/lng rounding
...
// Auto resend when user regains signal
window.addEventListener("online", () => {
  console.log("Network restored → flushing pending reports");
  flushOfflineQueue();
  updatePendingBadge();
});
...
function openReportPopup(entries) {
  ...
  if (entries.length === 1) {
    ...
    el.textContent = b.bird_id === "unringed"
      ? "Unberingter Vogel"
      : `${b.name} (${b.bird_id})`;
  } else {
...
function updateCoords(lat, lng) {
  document.getElementById("coords-display").textContent =
    `Lat: ${lat.toFixed(10)}, Lng: ${lng.toFixed(10)}`;
}

// ------------------------------------------------------------------------
// SAVE REPORTS → SERVER OR QUEUE
// ------------------------------------------------------------------------
...
async function saveSelectedReports() {
  ...
  if (!navigator.onLine) {
    alert("Kein Netz – Beobachtung offline gespeichert. Wird automatisch gesendet.");
  } else {
